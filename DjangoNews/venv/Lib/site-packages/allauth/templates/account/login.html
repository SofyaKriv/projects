...
<p>{% blocktrans with site.name as site_name %}Пожалуйста, войдите в систему с помощью одного из способов авторизации.
  Или <a href="{{ signup_url }}">зарегистрируйте</a>
на {{ site_name }} аккаунт и войдите:{% endblocktrans %}</p>
...
{% else %}
<p>{% blocktrans %}Если у Вас еще нет аккаунта, то сначала, пожалуйста,
